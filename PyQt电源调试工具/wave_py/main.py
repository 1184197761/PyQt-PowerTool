...
class RecvDataThread(QThread):
    ...
    def run(self):
        while True:
            if serial.inWaiting():
                if self.widget.get_recv_mode:
                    self.trigger.emit(list(serial.read(1024)))
                else:
                    self.trigger.emit(list(serial.read_until(b"\xEE")))
...
class MainWin(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.plotWidget = PlotWidget(background="w")
        self.plotWidget.showGrid(x=True, y=True)
        self.plotWidget.setLabel("left", "采样值")
        ...
        # self.plotWidget.setAutoPan(x=True)

        for channel in channels:
            channel.line = self.plotWidget.plot(
                name=channel.name,
                pen=channel.color,
    ...
    def reset_key(self):  # 复位键实现方法
        ...
        for cursor in self.cursors:
            self.plotWidget.addItem(cursor)
    ...
    def event_connect(self):
        ...
        self.displayTimer.timeout.connect(self.wave_display)
        ...
        self.spinBox_recvTimeOut.valueChanged.connect(self.recv_timeout)

    def get_recv_mode(self):
        return self.checkBox_recvTimeOut.isChecked()

    def recv_timeout(self,value):
        if self.checkBox_recvTimeOut.isChecked():
            serial.inter_byte_timeout = value / 1000

    def add_cursor(self):
        self.cursors = []
        for index in range(2):
            line = self.plotWidget.addLine(
                x=0,
                movable=True,
                pen="k",
                label="游标" + str(index),
    ...
    def dataHandle(self, rx):
        ...
        if rx[0] == 0xAA and rx[-1] == 0xEE:  # 判断首尾
            if len(self.channelX):
                ...
                self.channelX.append(self.channelX[-1] + 1)
            else:
                # self.startTime = time()
            ...
            for index in range(4):
                i = 4 * index
                rawData = (
                    (rx[1 + i] << 24) + (rx[2 + i] << 16) + (rx[3 + i] << 8) + rx[4 + i]
                )
                rawData = unpack("i", rawData.to_bytes(4, "little"))[0]
...
if __name__ == "__main__":
    ...
    serial = Serial(inter_byte_timeout=0.01)
